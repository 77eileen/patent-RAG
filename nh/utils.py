...
def generate_search_queries(
    ...
    Example:
        ...
        자동으로 생성되는 검색어:
        - a (단일 성분, include_single_ingredients=True 시)
        - a 미백
        - a 주름
        - a 화장품
        - a 화장
    ...
    search_queries = set()  # 중복 제거를 위해 set 사용
    ...
    # 2. 성분 + 용도 조합 (각 성분마다) - all_purposes 사용
    if all_purposes:
        for ingredient in ingredients:
    ...
    for size in range(min_combination_size, max_combination_size + 1):
        for combo in combinations(ingredients, size):
            ...
            # 성분 조합 + 각 용도 - all_purposes 사용
            if all_purposes:
                for purpose in all_purposes:
                    search_queries.add(" ".join(combo) + f" {purpose}")
    
    # 4. 모든 성분 + 모든 용도 - all_purposes 사용
...
def create_search_queries_from_components(
    ...
    Args:
        components_text (str): extract_components()로 추출된 텍스트
        min_combination_size (int): 최소 조합 크기
        max_combination_size (int): 최대 조합 크기
        include_single_ingredients (bool): 단일 성분 포함 여부
...
def search_patents_with_multiple_queries(
    ...
    #HIT 카운트 
    #HIT 카운트 = 해당 특허의 독립항이 여러 검색어에서 검색된 총 횟수


    all_patent_scores = {}  # {parent_id: [scores]}
    
    # 반복문 돌며 각 검색어로 벡터디비 검색 ㄱㄱ
    for idx, query in enumerate(search_queries, 1):
        ...
        try:
            ...
            # 검색 결과 돌며 특허당 HIT & 점수 계산
            for doc, score in results:
                if doc.metadata.get('claim_type') != 'independent':
                    continue #종속항은 무시
...
if __name__ == "__main__":
    ...
    # 사용자 질문
    query = "판토에아 아나나티스 I25 균주나 그 파쇄액, 배양액, 배양액의 추출물 없이 피부 장벽 강화와 주름 개선을 위한 화장품을 개발하려고 합니다. 이런 제품을 만들어도 괜찮을까요?"
